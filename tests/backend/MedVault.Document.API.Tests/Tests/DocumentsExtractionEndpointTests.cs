...
using MedVault.Document.API.Tests.Fixtures;
using Xunit;

namespace MedVault.Document.API.Tests.Tests;
...
public sealed class DocumentsExtractionEndpointTests : IClassFixture<MedVaultDocumentApiFactory>
{
    ...
    {
        _factory.StubPipeline.Handler = (_, _) => Task.FromResult(new ExtractionResult
        {
            IsMedical = true,
    ...
    {
        _factory.StubPipeline.Handler = (_, _) => throw new StructuredValidationException("Invalid JSON");
