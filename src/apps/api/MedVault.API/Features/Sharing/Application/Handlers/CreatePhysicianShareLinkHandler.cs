...
public sealed class CreatePhysicianShareLinkHandler
    : ICommandHandler<CreatePhysicianShareLinkCommand, ShareLinkManagementResponse>
{
    private readonly MedVaultDbContext _db;
    private readonly ISystemConfigurationService _systemConfigurationService;
    private readonly IShareProtectionService _shareProtection;
    private readonly ILogger<CreatePhysicianShareLinkHandler> _logger;

    public CreatePhysicianShareLinkHandler(
        MedVaultDbContext db,
        ISystemConfigurationService systemConfigurationService,
        IShareProtectionService shareProtection,
        ILogger<CreatePhysicianShareLinkHandler> logger)
    {
        _db = db;
    ...
    public async Task<ShareLinkManagementResponse> HandleAsync(
        CreatePhysicianShareLinkCommand command,
        CancellationToken cancellationToken = default)
    {
        ...
        var payload = SharePayloadMapper.CreatePhysicianPayload(command.Data);
        ...
        _logger.LogInformation(
            "Regular sharing link created for user {UserId} with share id {ShareId}",
            command.UserId,
            entity.Id);
